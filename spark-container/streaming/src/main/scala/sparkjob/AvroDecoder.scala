...
@SerialVersionUID(1L)
class AvroDecoder[+T >: Null <: IndexedRecord] private (var schema: Schema)
    extends Serializable {
  import sparkjob.AvroDecoder._
...
object AvroDecoder {
  ...
  def apply[T >: Null <: IndexedRecord](schema: Schema): AvroDecoder[T] =
    new AvroDecoder[T](schema)
}
