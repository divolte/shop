...
public class CatalogCategoryResource {
	...
	public void getCategory(
		...
		client.prepareSearch(CatalogEsConstants.CATALOG_INDEX).setQuery(QueryBuilders.constantScoreQuery(FilterBuilders.termFilter("categories", name)));
		CatalogEsConstants.execute(
				client.prepareSearch(CatalogEsConstants.CATALOG_INDEX)
				      .setQuery(QueryBuilders.constantScoreQuery(FilterBuilders.termFilter("categories", name)))
				      .addSort(SortBuilders.fieldSort("_id")) // Since we only support ID at this point
					...
					if (e.isPresent()) {
						...
						if (r.get().getHits().hits().length == 0) {
							...
							final List<Item> items = StreamSupport
								...
								.map(CatalogEsConstants::sourceToItem)
	...
	public static final class Category {
		...
		@JsonProperty("page") public final int page;
		@JsonProperty("size") public final int size;
		@JsonProperty("total") public final long total;
		@JsonProperty("items") public final List<Item> items;
